...
**Key risk:** `renderingModel: server-coupled` usually means this gap exists.
Check `internalApiRoutes` separately: Route Handlers are HTTP endpoints, but
their deployment URL, auth, cookies, CORS, and response shape still need review
for a non-browser mobile client.

---

## Next.js — Pages Router (`pages/` directory)

**Reusable:** Same as App Router above for utilities, types, and external API
clients. Deployed `pages/api/` routes are HTTP endpoints and may be callable by
mobile when their public base URL, auth model, and CORS behavior support it.
...
**API Needs:** If the frontend calls its own `/api/` routes, replace relative
URLs with the deployed absolute origin and review authentication, cookies,
CORS, rate limits, and versioning. Extract or replace an endpoint only when it
cannot safely serve a mobile client.
...
- DOM-dependent UI libraries → see `dependency-substitutions.md`.
