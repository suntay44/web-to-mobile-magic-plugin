...
| Web | Mobile equivalent | Difficulty | Notes |
|-----|------------------|------------|-------|
| `tailwindcss` | `nativewind` | config | Follow the NativeWind setup for the installed major version; Babel/Metro and CSS configuration differ across releases. Validate every reused class because DOM-specific utilities do not port. |
...
| `recoil` | `zustand` or `jotai` | rewrite | Recoil is no longer a good default for new work. Preserve state semantics and migrate atoms/selectors deliberately. |
...
| `document.cookie` | API-specific cookie support or `expo-secure-store` for tokens | rewrite | React Native does not expose the browser `document.cookie` API. Prefer token-based mobile auth in SecureStore; cookie-backed sessions need an HTTP client and server flow designed for native clients. |
...
| HTML `<video>` | `expo-video` or `react-native-video` | rewrite | `expo-av` video is deprecated; use Expo's dedicated video package for current Expo projects. |
| Web Audio / HTML `<audio>` | `expo-audio` | rewrite | `expo-av` audio is deprecated; update lifecycle, permissions, and background-playback configuration. |
